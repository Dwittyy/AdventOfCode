...
def formatpolicy(entry):
    splitentry = entry.split()

    minmax = splitentry[0].split("-") 

    return {
        "low": int(minmax[0]), 
        "high": int(minmax[1]), 
        "letter": splitentry[1][0], 
        "password": splitentry[-1]
    }
...
for entry in policies:
    if isvalid(formatpolicy(entry)):
        validpasswords += 1
    
print(validpasswords)
print(formatpolicy(policies[-1]))
print(isvalid(formatpolicy(policies[-1])))
print(formatpolicy(policies[-2]))
print(isvalid(formatpolicy(policies[-2])))